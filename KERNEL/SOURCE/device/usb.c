...
DWORD USBSetAddress(USB_COMMON *device)
{
	...
	req.I = 0;
	req.L = 0;
	DWORD cc = controller->XFER(device->PIPE, &req, 0);
	if (cc)
	{
...
DWORD ConfigureUSB(USB_COMMON *common)
{
	...
	req.I = 0;
	req.L = 8;
	DWORD cc = controller->XFER(common->PIPE, &req, &devinfo);
	if (cc)
	{
	...
	USB_CONFIG *config = 0;
	{
		...
		if (controller->XFER(common->PIPE, &req, &cfg))
		{
			goto LABEL001;
		}
		...
		req.L = cfg.TL;
		if (controller->XFER(common->PIPE, &req, config) || config->TL != cfg.TL)
		{
			HeapFree(config);
			config = 0;
		}
	...
	req.I = 0;
	req.L = 0;
	cc = controller->XFER(common->PIPE, &req, 0);
	if (cc)
	{
